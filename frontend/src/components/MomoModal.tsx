...
export const MomoModal: React.FC<MomoModalProps> = ({
    ...
    const initMomoPayment = async () => {
      ...
      try {
        const res = await api.createMomoUrl(
          orderId,
          amount,
          `Thanh toan don hang ${orderId} - SHOPBEE STORE AI`
        );
        if (isMounted) {
          if (res.payUrl) {
        ...
        if (isMounted) {
          console.warn("MoMo API create error, fallback to demo mode:", err.message);
          // Demo fallback URL
          setPayUrl(`https://test-payment.momo.vn/v2/gateway/pay?orderId=${encodeURIComponent(orderId)}&amount=${amount}`);
        }
      } finally {
        if (isMounted) {
          setIsLoading(false);
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/80 backdrop-blur-md animate-in fade-in duration-200 font-sans">
      <div className="relative w-full max-w-md bg-[#131c2e] border border-pink-900/50 rounded-3xl shadow-2xl overflow-hidden text-slate-200">
        ...
        <div className="p-4 bg-gradient-to-r from-[#a50064] via-[#d82d8b] to-[#131c2e] border-b border-pink-700/60 flex items-center justify-between">
          <div className="flex items-center gap-3">
            <div className="w-9 h-9 rounded-xl bg-white flex items-center justify-center shadow-md p-1 shrink-0">
              {/* MoMo Icon */}
              <div className="w-full h-full rounded-lg bg-[#a50064] flex items-center justify-center text-white font-black text-[11px] tracking-tighter">
                MOMO
              </div>
            </div>
            <div>
              <div className="flex items-center gap-2">
                ...
                <span className="px-1.5 py-0.5 rounded text-[9px] font-bold bg-white/20 text-white">SANDBOX</span>
              </div>
              <p className="text-[11px] text-pink-200">Mã đơn: <span className="font-mono font-semibold">{orderId}</span></p>
            </div>
          </div>
          <button
            onClick={onCancel}
        ...
        <div className="p-3.5 bg-[#0e1626] border-b border-slate-800 flex items-center justify-between text-xs">
          ...
          <div className="flex items-center gap-1.5 px-3 py-1.5 rounded-full bg-slate-800/80 border border-slate-700 text-slate-300 font-mono text-[11px]">
            <Clock className="w-3.5 h-3.5 text-pink-400 animate-pulse" />
            <span>{formatTimer(timeLeft)}</span>
          </div>
        </div>

        {/* Step 1: QR Scan & Sandbox Options */}
        {step === "qr_scan" && (
          <div className="p-6 space-y-5 text-xs">
            ...
            {isLoading ? (
              <div className="py-12 flex flex-col items-center justify-center space-y-3">
                ...
                <p className="text-slate-400 text-xs">Đang kết nối cổng MoMo Sandbox v2...</p>
              </div>
            ) : (
              <>
                {/* QR Code Card */}
                <div className="flex flex-col items-center justify-center p-5 rounded-2xl bg-white text-slate-900 shadow-xl border border-slate-200">
                  <div className="w-full flex items-center justify-between mb-3 text-[11px] font-semibold text-slate-600 border-b border-slate-100 pb-2">
                    <span className="flex items-center gap-1 text-[#a50064]">
                      <QrCode className="w-4 h-4" /> Quét mã MoMo QR
                    </span>
                    <span className="text-[10px] text-slate-400">Tự động nhận diện</span>
                  </div>

                  {/* QR Code Display */}
                  <div className="relative p-2 bg-white rounded-xl border-2 border-pink-500/30">
                    <img 
                      src={`https://api.qrserver.com/v1/create-qr-code/?size=180x180&data=${encodeURIComponent(payUrl || `momo://app?orderId=${orderId}&amount=${amount}`)}`}
                  ...
                  <p className="mt-3 text-[11px] text-slate-600 text-center font-medium">
                    Sử dụng ứng dụng <strong className="text-[#a50064]">Ví MoMo</strong> hoặc Camera để quét mã thanh toán
                  </p>
                </div>
                ...
                <div className="space-y-2">
                  <a
                    ...
                    className="w-full py-2.5 px-4 rounded-xl bg-gradient-to-r from-[#a50064] to-[#d82d8b] hover:from-[#880052] hover:to-[#be2178] text-white font-bold text-xs flex items-center justify-center gap-2 shadow-lg shadow-pink-900/30 transition-all active:scale-98"
                  >
                  ...
                  <div className="flex items-center gap-2">
                    ...
                    >
                      ...
                      <span>{copied ? "Đã chép link!" : "Sao chép liên kết thanh toán"}</span>
                    </button>

                    {/* Instant Simulator Button for Demo/Presentation */}
                    <button
                      type="button"
                      onClick={handleSimulatePayment}
                      title="Mô phỏng quét mã thành công để trình bày đồ án ngay trên máy tính"
                      className="py-2 px-3.5 rounded-xl bg-emerald-600/20 hover:bg-emerald-600/30 border border-emerald-500/40 text-emerald-300 font-bold text-[11px] flex items-center gap-1.5 transition-colors"
                    >
